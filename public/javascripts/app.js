...
class Budget {
	...
	// add a budget
	addBudget() {
		// create a budget object with the properties created from the constructor
		const budget = {
			desc: this.desc,
			amount: this.amount,
			type: this.type,
		...
		// 1. shift -> remove first item in an array
		// 2. unshift -> add item to the beginning of an array
		// 3. push -> add an item to the end of an array
...
class UI {
	...
	static loadBudgets() {
		if (Storage.getBudgets()) {
			...
			// total amounts for expense and income
...
document.addEventListener("DOMContentLoaded", e => {
	...
	// converting the nodelist of .fa-trash to Array
	// const deleteIcons = Array.from(document.querySelectorAll(".fa-trash"));
